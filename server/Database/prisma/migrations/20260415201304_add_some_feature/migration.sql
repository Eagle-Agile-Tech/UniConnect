/*
  ...
  - You are about to drop the column `type` on the `Community` table.
  - You are about to drop the column `status` on the `CommunityMember` table.
*/
...
-- Enforce constraint AFTER data is safe
ALTER TABLE "Institution"
  ALTER COLUMN "type" SET NOT NULL;

-- Now it's safe to drop old column
...
-- Foreign keys (safe guards)
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'Institution_userId_fkey'
  ) THEN
    ALTER TABLE "Institution"
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'Institution_verifiedById_fkey'
  ) THEN
    ALTER TABLE "Institution"
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'Institution_profileUserId_fkey'
  ) THEN
    ALTER TABLE "Institution"
      ADD CONSTRAINT "Institution_profileUserId_fkey"
      FOREIGN KEY ("profileUserId") REFERENCES "User"("id")
      ON DELETE SET NULL ON UPDATE CASCADE;
  END IF;
END $$;
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'InstitutionVerificationRequest_institutionId_fkey'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'InstitutionVerificationRequest_reviewedById_fkey'
  ) THEN
    ALTER TABLE "InstitutionVerificationRequest"
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'ExpertInvitation_institutionId_fkey'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'MessageReceipt_messageId_fkey'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'MessageReceipt_userId_fkey'
  ) THEN
    ALTER TABLE "MessageReceipt"
